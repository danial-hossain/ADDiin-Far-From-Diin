    ...
    public sealed class HadithService : IHadithService
    {
        ...
        public async Task<ScheduledHadith?> GetCurrentHadithAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                return await _context.ScheduledHadiths
                    .AsNoTracking()
                    .Where(h => h.SlotDate == now.Date && h.SlotTime == slot.Value)
                    .FirstOrDefaultAsync(cancellationToken);
            }
            catch (SqlException ex) when (ex.Number == 208)
            {
                _logger.LogWarning(ex, "ScheduledHadiths table is not available yet.");
                return null;
            }
        }
        ...
        public async Task EnsureCurrentHadithAsync(CancellationToken cancellationToken = default)
        {
            ...
            var exists = await _context.ScheduledHadiths
                .AnyAsync(h => h.SlotDate == now.Date && h.SlotTime == slot.Value, cancellationToken);
            if (exists)
            {
                return;
            }
            ...
            {
                _logger.LogWarning(ex, "Hadith slot was created by another application instance.");
        ...
        private async Task<(string Text, string? Source)?> GenerateHadithAsync(CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogWarning("Gemini API key is not configured. Set GEMINI_API_KEY.");
                return null;
            }
                ...
                contents = new[]
                {
                    new
                    {
                        parts = new[]
                        {
                            new
                            {
                                text = "Return one authentic hadith only as JSON with exactly these fields: text and source. " +
            ...
            try
            {
                ...
                {
                    _logger.LogError("Gemini hadith request failed with HTTP {StatusCode}: {Body}", response.StatusCode, body);
                    return null;
                }
                ...
                {
                    _logger.LogError("Gemini returned an empty hadith response.");
                    return null;
                }

                using var hadithDocument = JsonDocument.Parse(text);
                var hadithText = hadithDocument.RootElement.GetProperty("text").GetString();
                var source = hadithDocument.RootElement.GetProperty("source").GetString();
                if (string.IsNullOrWhiteSpace(hadithText) || string.IsNullOrWhiteSpace(source))
                {
                    _logger.LogError("Gemini returned an incomplete hadith payload.");
                    return null;
                }

                return (hadithText.Trim(), source.Trim());
            }
            catch (JsonException ex)
            {
                _logger.LogError(ex, "Gemini returned invalid JSON for the hadith.");
                return null;
            }
            catch (HttpRequestException ex)
            {
                _logger.LogError(ex, "Could not connect to Gemini for the scheduled hadith.");
                return null;
            }
        }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IHadithService>();
                    await service.EnsureCurrentHadithAsync(stoppingToken);
                }
                ...
                {
                    _logger.LogError(ex, "Scheduled hadith generation failed.");
                }
